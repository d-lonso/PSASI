...
const gCCKBundle = gBundleService.createBundle("chrome://cck-PSASI/content/cck.properties");
  ...
  observe: function observe(subject, topic, data) {
    switch (topic) {
      ...
      case "final-ui-startup":
        init();
        if (Cc["@mozilla.org/extensions/manager;1"]) {
          var location = em.getInstallLocation("psasi@d-lonso").getItemLocation("psasi@d-lonso");
          installXPIs(location);
        } else {
          Components.utils.import("resource://gre/modules/AddonManager.jsm");
          AddonManager.getAddonByID("psasi@d-lonso", function(addon) {
            if (addon) {
...
function init() {
  ...
  if (gPrefBranch.prefHasUserValue("extensions.psasi@d-lonso.version")) {
    installedVersion = gPrefBranch.getCharPref("extensions.psasi@d-lonso.version");
  }

  if (!gPrefBranch.prefHasUserValue("extensions.psasi@d-lonso.install-event-fired")) {
    firstrun = true;
    gPrefBranch.setBoolPref("extensions.psasi@d-lonso.install-event-fired", true);
  }
  ...
  if (firstrun || extVersion != installedVersion)
  {
    ...
    gPrefBranch.setCharPref("extensions.psasi@d-lonso.version", extVersion);
  }
}
...
function uninit() {
  if (gPrefBranch.prefHasUserValue("extensions.psasi@d-lonso.install-event-fired")) {
    gPrefBranch.clearUserPref("extensions.psasi@d-lonso.install-event-fired");
  }

  if (gPrefBranch.prefHasUserValue("extensions.psasi@d-lonso.version")) {
    gPrefBranch.clearUserPref("extensions.psasi@d-lonso.version");
  }
...
function addCertificate(CertName, CertTrust) {
  ...
  var channel = gIOService.newChannel("chrome://cck-PSASI/content/" + CertName, null, null);
  ...
  if (bookmarksToRemove.indexOf("firefoxFolder") != -1) {
    ...
    if ((firefoxFolder != -1) && (bookmarks.getItemType(firefoxFolder) == bookmarks.TYPE_FOLDER)) {
      ...
      if (aboutMozilla != -1 &&
          ...
          /http:\/\/www.mozilla.com\/.*\/about/.test(bookmarks.getBookmarkURI(aboutMozilla).spec))
